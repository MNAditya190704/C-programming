...
/*            Character received
     ...
     '*'       '/'     Yes       No
      │         │       │         │
      ▼         ▼       ▼         ▼
 Multi-line   Single  Preproc.  Continue
 Comment      Comment State     checking*/
pevent_t * pstate_idle_handler(FILE *fd, int ch)
{
	int pre_ch;
	switch(ch)
...
No        Yes
 │         │
 ▼         ▼
Continue  Compare with
reading   keyword table
               │
      ┌────────┴────────┐
      │                 │
      ▼                 ▼
...
pevent_t * pstate_reserve_keyword_handler(FILE *fd, int ch)
{
    ...
    else
    {
        ...
        int res = is_reserved_keyword(word);

        fseek(fd,-1L,SEEK_CUR);

        if(res)
        {
            pevent_data.property = res;
